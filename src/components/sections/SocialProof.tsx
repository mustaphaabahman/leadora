...
export default function SocialProof() {
  const { t } = useLanguage();
  const stats = t("social_proof", "stats") as { value: string, label: string, desc?: string }[];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20">
          {stats.map((stat, i) => (
            <motion.div 
              ...
              className="text-center p-8 glass rounded-2xl border border-white/5 flex flex-col items-center justify-center"
            >
              <div className="text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#00f0ff] to-[#b026ff] mb-3">{stat.value}</div>
              <div className="text-white/90 font-bold tracking-wider mb-2">{stat.label}</div>
              {stat.desc && <div className="text-white/50 text-sm font-medium leading-relaxed">{stat.desc}</div>}
            </motion.div>
          ))}
        </div>
