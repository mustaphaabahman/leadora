...
function Counter({ value, suffix = "", title, type = "counter" }: { value: string | number; suffix?: string; title: string; type?: string }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: "-100px" });
  const [count, setCount] = useState(0);

  useEffect(() => {
    if (isInView && type === "counter") {
      let start = 0;
      const duration = 2000;
      const val = parseInt(value.toString(), 10) || 24;
    ...
    if (type === "scale") {
      return (
        <motion.div
          ...
          animate={isInView ? { scale: [1.2, 1], opacity: 1 } : {}}
      ...
      return (
        <div className="relative inline-flex items-center justify-center">
          <motion.div
            ...
            className="absolute -inset-4 rounded-full border border-dashed border-[#00f0ff]/50"
          />
    ...
    if (type === "infinity") {
      return (
        <motion.div
          ...
          className="inline-block bg-gradient-to-r from-[#00f0ff] via-[#b026ff] to-[#00f0ff] bg-[length:200%_auto] text-transparent bg-clip-text"
        >
          {value}{suffix}
        </motion.div>
      );
    }
    // Default counter (for 24/7)
    return <span>{count}{value.toString().includes('/') ? '/7' : ''}{suffix}</span>;
  };

  return (
    <div ref={ref} className="text-center p-4 md:p-6 glass rounded-2xl border border-white/5 relative overflow-hidden group h-full flex flex-col justify-center">
          ...
          <div className="lg:w-1/2">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-4 mb-4">
              {whyChooseData?.metrics?.map((metric: any, idx: number) => (
                <Counter key={idx} value={metric.val} suffix={metric.suf} title={metric.label} type={metric.type} />
              ))}
            </div>
            {whyChooseData?.metrics_disclaimer && (
              <div className="text-white/40 text-xs mt-2 italic text-center md:text-left">
                {whyChooseData.metrics_disclaimer}
              </div>
            )}
          </div>
            ...
            >
              ...
              <div className="relative z-10 w-full max-w-[92%] md:max-w-md p-2 md:p-6 scale-95 md:scale-100 mx-auto">
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4 md:p-6 shadow-2xl relative overflow-visible md:overflow-hidden group">
                  ...
                  >
                    {whyChooseData?.hologram?.top_badge}: {whyChooseData?.hologram?.top_label}
                  </motion.div>
                  ...
                  >
                    {whyChooseData?.hologram?.bottom_badge}: {whyChooseData?.hologram?.bottom_label}
                  </motion.div>
                  
                  <div className="flex justify-between items-center mb-4 md:mb-6">
                    <div className="max-w-[70%]">
                      <div className="text-white/50 text-[10px] md:text-sm font-medium mb-1 truncate">{whyChooseData?.hologram?.main_label}</div>
                      <div className="text-lg md:text-3xl font-bold text-[#fff] flex items-center gap-1 md:gap-2 flex-wrap">
                        {whyChooseData?.hologram?.main_value}
                        <span className="text-[9px] md:text-xs bg-[#00f0ff]/20 text-[#00f0ff] px-1.5 py-0.5 md:px-2 md:py-1 rounded-full flex items-center whitespace-nowrap">
                          {whyChooseData?.hologram?.sec_badge}
                  ...
                  <div className="grid grid-cols-2 gap-2 md:gap-4 mt-4 md:mt-6 pt-4 md:pt-6 border-t border-white/5">
                    <div>
                      <div className="text-white/40 text-[9px] md:text-xs mb-0.5 md:mb-1 truncate">{whyChooseData?.hologram?.top_label}</div>
                      <div className="text-white text-sm md:text-base font-semibold">{whyChooseData?.hologram?.top_badge} <span className="text-[#00ff88] text-[10px] md:text-xs ml-0.5 md:ml-1">↑</span></div>
                    </div>
                    <div>
                      <div className="text-white/40 text-[9px] md:text-xs mb-0.5 md:mb-1 truncate">{whyChooseData?.hologram?.bottom_label}</div>
                      <div className="text-white text-sm md:text-base font-semibold">{whyChooseData?.hologram?.bottom_badge} <span className="text-[#00f0ff] text-[10px] md:text-xs ml-0.5 md:ml-1">↑</span></div>
                    </div>
                  </div>
                </div>
              </div>
